...
class PreprocessedReader(Reader):
    ...
    def read(self, list_of_sentence_tuples):
        sentences = []
        #for sentence_id, sentence in enumerate(list_of_sentence_tuples): commented by codetild
        for sentence_id, sentence in enumerate(list_of_sentence_tuples): #added by codetild
            words = [word for (word, pos_tag) in sentence[0]]
            pos_tags = [pos_tag for (word, pos_tag) in sentence[0]]
            #stems = [stem for stem in sentence[1]] #added by codetild
            shift = 0
            sentences.append(Sentence(
                words=words,
                pos=pos_tags,
                stems=sentence[1] #added by codetild: the stems have come from the hazm stemming tools. You may also ignore stemming 
                # option and try stems = words 
            ))
